	...
	function initNpm(data, fileName) {
		...
		exec(`npm i ${data.source}`, (err, result) => {
			...
			process.on('exit', () => {
				...
				delDir(nodeModulesPath);
				delDir(packageLockPath);
			})
			...
			const tplFilePath = path.resolve(process.cwd(), `./node_modules/${data.source}`);
			let projectPath = '';
			if (typeof (fileName) == 'string') {
				projectPath = path.resolve(process.cwd(), `./${fileName}`);
			} else {
				projectPath = path.resolve(process.cwd(), `./${data.source}`);
			}
